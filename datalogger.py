...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.scanAct = QAction(
            QIcon.fromTheme('document-prop erties'), self.langstr[5], self)
    ...
    def startlogging(self):
        ...
        if not self.isRunning:              # not running ?
            ...
            self.serport.write(bytes(tmpstr))  # send configuration to arduino
            self.serport.readline().decode('ascii')  # DO NOT REMOVE tHIS LINE
            self.serport.write(bytes(b'R'))    # send start command to arduino
    ...
    def about_app(self):
        """ About dialog box """
        QMessageBox.about(self, self.langstr[8],
                          "The poorman's data logger based on Arduino !!! \
